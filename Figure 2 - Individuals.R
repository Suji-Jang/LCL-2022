...
lcl_dat <- data.frame(read.csv("LCL_data_040722.csv"))
...
gm.gsd.df <- read.csv("GM_GSD_data.csv",as.is=TRUE)
POD.med.chem <- gm.gsd.df[,paste("Chem",1:42,"GM",sep=".")]
POD.sens05.chem <- gm.gsd.df[,paste("Chem",1:42,"Sens.05",sep=".")]
...
POD.sens05.chem <- POD.sens05.chem[,col.300]
...
# Collecting EC10 for each individual
...
ind.med.df <- as.data.frame(matrix(NA,nrow=146,ncol=32))
for (chemnum in 1:32){
  temp.ind.med <- apply(as.data.frame(pod.chem.ind.list[chemnum]),2,median)
  ind.med.df[,chemnum] <- temp.ind.med
}
ind.med.df <- t(ind.med.df)
...
# CA for individual
ca.ind.med.df <- as.data.frame(matrix(NA,nrow=146,ncol=8))
for (cellnum in 1:146){
  temp.ind.ca <- t(mix.info[,"Mix.1"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,1] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.2"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,2] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.3"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,3] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.4"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,4] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.5"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,5] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.6"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,6] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.7"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,7] <- temp.ind.ca
  temp.ind.ca <- t(mix.info[,"Mix.8"]) %*% as.matrix(1/(ind.med.df[,cellnum]))
  ca.ind.med.df[cellnum,8] <- temp.ind.ca
}
colnames(ca.ind.med.df) <- paste0("Mix.",1:8)
# Inverse CA POD to CA POD
ca.ind.med.df <- 1/ca.ind.med.df
...
for (mixnum in 1:8){
  ...
  temp.df <- data.frame(Mix=rep(mix.name,146),CA.EC10.med=ca.ind.med.df[,mixnum],Mea.EC10.med=ind.mix.med.df[,mixnum])
...
# AI
ai.ind.med.df <- ind.mix.med.df/ca.ind.med.df
colnames(ai.ind.med.df) <- paste0("Mix.",1:8)

ai.ind.med.melted <- melt(ai.ind.med.df)
...
##### Figure 2B - Plotting boxplot for each mixture #####
...
p1.2 <- ggarrange(p1,p2)

# Plotting boxplot for each individual
ai.ind.med.df.trans <- as.data.frame(t(ai.ind.med.df))
colnames(ai.ind.med.df.trans) <- cells
ai.ind.med.trans.melted <- melt(ai.ind.med.df.trans)

ggplot(ai.ind.med.trans.melted,aes(x=value,y=reorder(variable, value, FUN = median))) + geom_boxplot(outlier.shape = NA) +
  scale_x_log10(breaks=c(0.001,0.01,0.1,1,10)) + theme_classic() + geom_vline(xintercept=1,linetype="dashed") +
...
heatmap.df <- as.matrix(log10(ai.ind.med.df.trans))
colnames(heatmap.df) <- cells
rownames(heatmap.df) <- c("AC50-L","AC50-H","POD-L","POD-H","Expo-L","Expo-H","RfD-L","RfD-H")
pdf("Figure 2C - Heatmap_070622.pdf",height=5,width=20)
p3 <- heatmap.2(heatmap.df,trace="n",cexRow=1,scale="none",lhei=c(2,8.5),lwid=c(0.5,8.5),col= warmcool_hcl,
